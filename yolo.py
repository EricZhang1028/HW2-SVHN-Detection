...
class YOLO(object):
    _defaults = {
        "model_path": 'logs/svhn_weights/ep009-loss29.924-val_loss27.476.h5',
    ...
    def detect_image(self, image, single_image=True, output=list(), bbox_set=list(), label_set=list(), score_set=list(), speed=list()):
        ...
        for i, c in (list(enumerate(out_classes))):
            ...
            x = float(old_left)
            y = float(old_top)
            width = float(old_right - old_left)
            height = float(old_bottom - old_top)
            
            output_str='{} {} {} {} {}'.format(label, x, y, width, height)
            output.append(output_str)
            bbox_set.append(([x, y, width, height]))
